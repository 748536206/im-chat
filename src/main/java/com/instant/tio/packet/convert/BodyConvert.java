package com.instant.tio.packet.convert;


import com.instant.tio.inter.MsgType;
import com.instant.tio.packet.ChatRequestBody;
import com.instant.tio.packet.ContextUser;
import com.instant.tio.packet.LayimConst;
import com.instant.tio.packet.LayimToClientChatMsgBody;
...
public class BodyConvert {
    ...
    public LayimToClientChatMsgBody convertToClientMsgBody(ChatRequestBody requestBody, ChannelContext channelContext){
         ...
         * */
        if(requestBody.getMtype()== MsgType.CLIENT_TO_CLIENT){
            msgBody.setId(contextUser.getUserid());
            //消息类型：好友
            msgBody.setType(LayimConst.CHAT_TYPE_FRIEND);
        }else if(requestBody.getMtype() == MsgType.CLIENT_TO_GROUP){
